...
public class CardManagementUI {
		...
		while (!success) {
			...
			if (check) {
				if (type.equals("Silver")) {
					...
					System.out.println(customService.requestDebitCardUpgrade(debitCardNumber, mString));
					...
					System.out.println(customService.requestDebitCardUpgrade(creditCardNumber, mString));
	...
	void reportDebitStatementMismatch() {
		...
		while (!success) {
			...
			try {
				...
				if ((transactionId = scan.next()).equals("x"))
					return;
	...
	void viewQueryStatus() {
		...
		while (!success) {
			...
			try {
				if ((customerReferenceId = scan.next()).equals("x"))
					return;
	...
	void replyQueries() {
		...
		if (check) {
			...
			if ((newStatus = scan.next()).equals("x"))
				return;
